...
int precedenza ( int indicestrada ) {
	...
	/* verifico se ci sono auto a cui dare la precedenza */
	int i;
	...
	fflush(stdout);
	#endif
	/* guardo se ci sono auto nelle strade di indice minore del mio
	   a cui devo dare precedenza */
	for( i=0; i<indicestrada; i++ )
		if( AutoInAttesa[i] > 0 )
			return(0); /* devo dare precedenza a qualcuno */
...
{ 
	...
	while ( 
		...
		/* utilizzare ANCHE la funzione  precedenza */
		 ! (
			AutoInIncrocio<2
			&&
			mioturno==Turno[indicestrada]
			&&
			precedenza(indicestrada)  
		  )
		) 
	...
	*/
	...
	AutoInIncrocio--;
	DBGpthread_cond_broadcast(&cond,Plabel);
	printf("auto %s esce dall'incrocio %i \n", Plabel, mioturno ); 
	fflush(stdout);
	DBGpthread_mutex_unlock(&mutex,Plabel); 
